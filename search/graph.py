...
class Graph:
    ...
    def __init__(self, filename=None, input_dict = None):
        ...
        if filename != None and input_dict != None: #if both filename and input_dict included as args raise error
            raise ValueError("Expected only one input! Choose between a dictionary or file path!")

        if filename == None and input_dict == None: #if neither filename nor input_dict included as args raise error
            raise ValueError("Include either a filename or a dictionary as a graph input")
